...
from .vtt_service import VttResources, load_vtt_resources_from_root
...
def _resource_root() -> Path:
    ...
    raise FileNotFoundError("packaged VTT parser resources are unavailable")
...
def _dependency_lock_source(resource_root: Path) -> Path:
    ...
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    raise FileNotFoundError("packaged VTT dependency lock is unavailable")


def _read_request() -> dict[str, object]:
    header = sys.stdin.buffer.read(8)
    if len(header) != 8:
        raise ValueError("VTT worker request length prefix is missing")
    length = struct.unpack(">Q", header)[0]
    if length <= 0 or length > MAX_REQUEST_BYTES:
        raise ValueError("VTT worker request length is invalid")
    payload = sys.stdin.buffer.read(length)
    if len(payload) != length or sys.stdin.buffer.read(1):
        raise ValueError("VTT worker request framing is invalid")
    parsed = json.loads(payload.decode("utf-8"))
    if canonical_json(parsed) != payload or not isinstance(parsed, dict):
        raise ValueError("VTT worker request is not canonical JSON")
    required = {
        "protocol_version",
        "parser_id",
    ...
    if set(parsed) != required:
        raise ValueError("VTT worker request fields diverge from the fixed protocol")
    return parsed
...
def _validate_request(
    ...
    *,
    operation_root: Path,
    resources: VttResources,
) -> Path:
    if request["protocol_version"] != VTT_WORKER_PROTOCOL_VERSION:
        raise ValueError("VTT worker protocol version mismatch")
    if request["parser_id"] != VTT_PARSER_ID:
        raise ValueError("VTT worker parser identity mismatch")
    if (
        require_sha256(str(request["implementation_sha256"]))
        != resources.implementation_sha256
    ):
        raise ValueError("VTT worker implementation hash mismatch")
    if request["security_profile_id"] != VTT_SECURITY_PROFILE_ID:
        raise ValueError("VTT worker security profile mismatch")
    if (
        require_sha256(str(request["config_sha256"]))
        != resources.config_sha256
    ):
        raise ValueError("VTT worker configuration hash mismatch")
    if (
        request["verified_input_relative_name"] != INPUT_NAME
        or request["output_filename"] != OUTPUT_NAME
    ):
        raise ValueError("VTT worker filenames are not fixed")
    input_path = operation_root / INPUT_NAME
    if not input_path.is_file() or input_path.is_symlink():
        raise ValueError("verified VTT worker input is unavailable")
    ...
    if (
        ...
        raise ValueError("verified VTT worker input size mismatch")
    if _sha256_file(input_path) != require_sha256(
        str(request["verified_input_sha256"])
    ):
        raise ValueError("verified VTT worker input hash mismatch")
    return input_path
...
def main() -> int:
    ...
    try:
        ...
        candidate = validate_vtt_candidate(candidate, input_bytes=input_bytes)
